"""
train_mtlm.py — Self-supervised Masked Tabular Language Modelling pretraining.

Plan §8.5 / Novelty N4 / Ablation A15.

This is the Phase 6A pretraining loop. It trains a shared encoder (the
exact same :class:`FeatureEmbedding` + :class:`TransformerEncoder` stack
used by :class:`model.TabularTransformer`) to predict the original values
of randomly masked feature tokens, using the 80/10/10 BERT-style masking
already implemented by :class:`tokenizer.MTLMCollator`. Once pretraining
converges, the encoder state dict is persisted to a checkpoint file that
downstream supervised training (``src/train.py --pretrained-encoder
PATH``) picks up for fine-tuning with the Plan §8.5.5 two-stage learning
rate.
...
.. code-block:: bash
    ...
    poetry run python src/train_mtlm.py \\
        --seed 42 \\
        --output-dir results/mtlm/run_42 \\
        --d-model 32 --n-heads 4 --n-layers 2 \\
        --use-temporal-pos \\
        --temporal-decay-mode scalar \\
        --feature-group-bias-mode scalar \\
        --dropout 0.1 \\
        --mask-prob 0.15 --min-mask 1 --max-mask 5 \\
        --lr 3e-4 --weight-decay 1e-5 --warmup-frac 0.10 \\
        --epochs 50 --patience 10 --batch-size 256

Outputs (under ``--output-dir``)
-------------------------------

* ``config.json``             — resolved argparse + runtime metadata
* ``pretrain_log.csv``        — per-epoch train + held-out loss + components
* ``best.pt``                 — hardened checkpoint bundle (full model)
* ``best.pt.weights``         — weights-only sidecar (safe load path)
* ``encoder_pretrained.pt``   — encoder+embedding-only state dict —
                                 consume via ``train.py --pretrained-encoder``

Rationale — why do MTLM?
------------------------

The coursework PDF frames the deliverable as a "small transformer-based
**language model**". A supervised-only training pipeline technically
satisfies the "explicit self-attention" requirement but misses the
defining feature of the LLM paradigm: self-supervised pretraining on
abundant unlabelled signal, followed by task-specific fine-tuning on
scarce labels. Adapting that two-stage recipe to structured credit data
is the single most "LM-like" component of the project (Plan §1.6 / §8.5).

Rubachev et al. (2022) — "Revisiting Pretraining Objectives for Tabular
Deep Learning" — find that masked-value reconstruction consistently
improves downstream performance on medium-scale tabular problems,
particularly when the supervised signal is noisy (our 3.5:1 class
imbalance + focal loss regime fits that profile). Whether the effect
replicates on *this* dataset is an Ablation A15 question.
"""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="MTLM self-supervised pretraining (Plan §8.5 / Novelty N4).",
    ...
    # Architecture — mirrors the downstream TabularTransformer so the
    # pretrained encoder is drop-in compatible.
    ...
    g.add_argument("--mask-prob", type=float, default=0.15,
                   help="Per-token selection probability (Plan §5.4B / §8.5.2).")
    ...
    g.add_argument("--max-mask", type=int, default=5,
                   help="Upper bound on masked tokens per row (caps difficulty).")
    ...
    g.add_argument("--replace-with-random", type=float, default=0.1,
                   help="Fraction replaced with random valid value (BERT 80/10/10).")
    ...
    # Optimiser + schedule (mirrors train.py)
...
def _load_splits(
    ...
    """Load train + val splits + metadata.

    MTLM pretraining uses the train set for the primary loss and the val
    set for early stopping on held-out reconstruction loss — NEVER the
    test set (test is reserved for supervised evaluation only)."""
...
def build_cosine_warmup_schedule(
    ...
    """Same schedule as `src/train.py` — keeps pretraining and fine-tuning
    on a single, well-tested LR-ramp implementation."""
    if total_steps <= 0:
...
def build_mtlm_model(
    ...
    """Instantiate embedding + encoder + MTLMHead with drift-safe layouts.

    The encoder architecture here must mirror the downstream
    :class:`model.TabularTransformer` exactly so the pretrained state dict
    is drop-in for supervised fine-tuning."""
    # Embedding — use_mask_token must be True for the MASK path to fire.
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    logger.info("=" * 72)
    logger.info("MTLM pretraining (Plan §8.5 / Novelty N4) — seed %d", args.seed)
    ...
    # Separate MTLMCollators so train/val draw masks independently.
    ...
    val_loader = make_loader(
        ...
        mtlm=val_collator,
        num_workers=args.num_workers,
        # Keep the residual tail — on the actual val split this only
        # matters when val_size < batch_size (smoke-test mode); in normal
        # runs the loader has ≥ 17 batches so drop_last=True/False are
        # equivalent in practice but False avoids silently yielding zero
        # batches.
        drop_last=False,
    )
    ...
    # Precompute per-numerical-feature variance for MSE normalisation.
    if args.no_variance_normalise:
        ...
        logger.info(
            "Variance-normalising numerical MSE (mean σ² = %.3f across 14 features)",
    ...
    # ── Persist full checkpoint + encoder-only side artefact ─────────────
    save_checkpoint(
        output_dir / "best.pt",
        model,
        optimizer=optimizer,
    ...
    # Encoder-only artefact: embedding.* + encoder.* (see
    # :meth:`MTLMModel.encoder_state_dict`). Consumed directly by
