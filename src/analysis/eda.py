"""Exploratory data analysis for the UCI credit-default dataset (§2 of report).

Produces the twelve figures and the per-feature summary-statistics table
that back the EDA chapter. All figures are 300 DPI, serif-font, CVD-safe
Okabe-Ito palette, top/right spines off -- matches the journal
house-style set by :func:`set_publication_style`.

What each figure answers, in one line each:

* fig01 -- how imbalanced is the target? (class counts + proportions)
* fig02 -- does default rate depend on SEX / EDUCATION / MARRIAGE? (chi-squared per feature)
* fig03 -- LIMIT_BAL / AGE distributions by default status (Mann-Whitney U + rank-biserial r)
* fig04 -- PAY_0 semantics: motivates the hybrid categorical / ordinal tokenisation (N1)
* fig05 -- 6-month trajectories of PAY / BILL / PAY_AMT split by default status (+ 95% CIs)
* fig06 -- credit utilisation: Sep distribution + 6-month trajectory
* fig07 -- full Pearson correlation heatmap (lower triangle) -- motivates cross-feature attention
* fig08 -- feature-to-target association ranking (|r_pb| for numeric, Cramer's V for categorical)
* fig09 -- BILL_AMT temporal structure: cross-month corr + autocorr decay vs lag
* fig10 -- pairwise interaction plots (util, PAY x LIMIT, default rate over age x education)
* fig11 -- Aug -> Sep PAY transition heatmaps (sequential dependency for attention to exploit)
* fig13 -- repayment ratio analysis (Sep distribution + 6-month medians).

Data loading goes through :mod:`src.data.preprocessing` so the EDA and the
modelling pipeline see identical cleaned inputs.
"""
...
def set_publication_style():
    """Set matplotlib rcParams for the report house-style.

    300 DPI save, 150 DPI on-screen, serif-family fonts, top/right
    spines off (Tufte style). Called once at the start of :func:`run_eda`
    so every figure this module produces inherits the same look.
    """
    plt.rcParams.update(
        {
            "figure.dpi": 150,
...
def plot_class_distribution(df: pd.DataFrame, save_dir: str):
    """Figure 01: class counts + class proportions with imbalance ratio.

    Two-panel: bar chart of raw counts (left) and pie of proportions
    (right). Suptitle reports the imbalance ratio -- ``~3.5 : 1`` here,
    which frames the class-imbalance discussion for the rest of the
    report (justifies stratified splits, pos-weighted BCE, etc.).
    """
...
def plot_categorical_by_target(df: pd.DataFrame, save_dir: str):
    """Figure 02: default rate per SEX / EDUCATION / MARRIAGE level.

    One stacked-bar subplot per categorical feature, default rate
    annotated in-cell, chi-squared statistic and p-value in the subplot
    title. Flags the (small) demographic dependence that motivates the
    fairness audit in :mod:`src.evaluation.fairness`.
    """
    cat_features = {
        "SEX": {1: "Male", 2: "Female"},
    ...
    for idx, (feat, label_map) in enumerate(cat_features.items()):
        ...
        ax.bar(
            x, 1 - default_rates, color=C_ND, label="No Default", edgecolor="white", linewidth=0.5
        )
        ax.bar(
            x,
            default_rates,
            bottom=1 - default_rates,
            color=C_D,
            label="Default",
            edgecolor="white",
            linewidth=0.5,
    ...
    fig.suptitle(
        "Categorical Features: Default Rate by Category", fontsize=13, fontweight="bold", y=1.02
    )
    plt.tight_layout()
...
def plot_numerical_distributions(df: pd.DataFrame, save_dir: str):
    """Figure 03: LIMIT_BAL / AGE density by default status.

    Histogram + KDE overlay for each. Titles carry the Mann-Whitney U
    p-value and the rank-biserial ``r_rb`` (effect size, in ``[-1, 1]``).
    Mann-Whitney rather than a t-test because both features are
    heavy-tailed and the t-test would be mis-calibrated.
    """
    ...
    fig.suptitle(
        "Numerical Feature Distributions by Default Status", fontsize=13, fontweight="bold", y=1.02
    )
    plt.tight_layout()
...
def plot_pay_status_analysis(df: pd.DataFrame, save_dir: str):
    """Figure 04: PAY_0 semantics in three panels.

    (a) PAY_0 value distribution by default status.
    (b) Default rate per PAY_0 value, showing the non-linear risk
        profile: default rate is flat-ish for {-2, -1, 0} (categorical
        "no bill / paid / current" states) and rises steeply for
        positive PAY_0 (ordinal delinquency months).
    (c) Heatmap of PAY-status proportions across the six months by
        default class.

    Motivates the hybrid categorical-ordinal PAY tokenisation (novelty N1).
    """
    ...
    ax.annotate(
        "No bill",
        xy=(0, d0_counts.iloc[0]),
        xytext=(-0.5, d0_counts.iloc[0] * 1.1),
        fontsize=7,
        fontstyle="italic",
        color="gray",
    )
    ...
    for target_val, label in [(0, "No Default"), (1, "Default")]:
        ...
        for col, month in zip(PAY_STATUS_FEATURES, MONTH_LABELS):
            ...
            proportions = (
                subset[col].value_counts(normalize=True).reindex(all_pay_vals, fill_value=0)
            )
    ...
    fig.suptitle(
        "PAY Feature Semantic Analysis - Motivating Tokenisation Design",
        fontsize=14,
        fontweight="bold",
        y=1.01,
    )
...
def plot_temporal_trajectories(df: pd.DataFrame, save_dir: str):
    """Figure 05: 6-month mean trajectories with 95 % CIs, per default class.

    One subplot per feature group (PAY, BILL_AMT, PAY_AMT). The diverging
    trajectories (defaulters flat-lining while non-defaulters recover)
    motivate the temporal positional encoding + decay biases in the model
    (novelties N2 / N3).
    """
...
def plot_utilisation_analysis(df: pd.DataFrame, save_dir: str):
    """Figure 06: credit utilisation (BILL_AMT / LIMIT_BAL) analysis.

    (a) September utilisation distribution by default class, with 100 %
        annotated as the visible tipping point.
    (b) Mean utilisation ratio across all six months. Defaulters sit
        above 1.0 consistently, non-defaulters below.
    """
    ...
    fig.suptitle(
        "Credit Utilisation Analysis - A Key Risk Indicator", fontsize=13, fontweight="bold", y=1.02
    )
    plt.tight_layout()
...
def plot_correlation_heatmap(df: pd.DataFrame, save_dir: str):
    """Figure 07: lower-triangle Pearson-r heatmap over features + target.

    Annotated diverging colour map (``RdBu_r``, centred at 0). The BILL_AMT
    block (6x6 lower-right quadrant) is strongly positively correlated
    within itself -- the smoking gun for self-attention across time steps.
    """
    numeric_cols = [
        c for c in ALL_FEATURE_COLS if c in df.select_dtypes(include=[np.number]).columns
    ]
...
def plot_feature_target_association(df: pd.DataFrame, save_dir: str):
    """Figure 08: feature-to-target association strength, ranked.

    Uses point-biserial ``|r_pb|`` for numerical features (including
    PAY_*, which are treated as ordinal here even though the model
    treats them as hybrid categorical-ordinal) and Cramer's V for the
    pure-categorical features. Bars colour-coded by feature family.
    """
...
def plot_bill_amt_autocorrelation(df: pd.DataFrame, save_dir: str):
    """Figure 09: BILL_AMT temporal structure.

    (a) Cross-month Pearson correlation heatmap.
    (b) Mean autocorrelation vs lag, split by default class. Both classes
        show slow decay (~0.85 at lag 5), so a memory-free bag-of-features
        model loses information -- the transformer's full self-attention
        recovers it.
    """
    ...
    fig.suptitle(
        "Bill Amount Temporal Structure - Motivating Self-Attention over Time Steps",
        fontsize=12,
        fontweight="bold",
        y=1.02,
    )
...
def plot_feature_interactions(df: pd.DataFrame, save_dir: str):
    """Figure 10: three pairwise interactions self-attention should capture.

    (a) LIMIT_BAL x BILL_AMT1 scatter with 100 % utilisation diagonal.
    (b) LIMIT_BAL boxplot per PAY_0 value, box colour = default rate.
    (c) Default rate across age bins, one line per education level.

    Each panel is a non-linear or conditional pattern a tree model must
    hand-engineer but self-attention can learn directly.
    """
    fig, axes = plt.subplots(1, 3, figsize=(16, 5))
    ...
    ax.scatter(
        sample[sample[TARGET_COL] == 0]["LIMIT_BAL"],
    ...
    ax.scatter(
        sample[sample[TARGET_COL] == 1]["LIMIT_BAL"],
    ...
    bp = ax.boxplot(
        data_boxes, positions=range(len(pay0_vals)), widths=0.6, patch_artist=True, showfliers=False
    )
    ...
    fig.suptitle(
        "Feature Interactions - Pairwise Patterns Self-Attention Can Capture",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def plot_pay_transitions(df: pd.DataFrame, save_dir: str):
    """Figure 11: PAY_2 -> PAY_0 transition probabilities per default class.

    Row-normalised crosstab (``P(PAY_0 | PAY_2)``) clipped to ``[-2, 4]``
    so the tail values don't dilute the readable centre. The defaulter
    panel is visibly biased toward the upper-right (delinquency
    escalates) while the non-defaulter panel concentrates on the
    diagonal + stability.
    """
    ...
    fig.suptitle(
        "Payment Status Transitions - Sequential Dependencies\n"
...
def generate_summary_statistics(df: pd.DataFrame, save_dir: str):
    """Per-feature descriptive statistics split by default class.

    Writes both CSV (for programmatic consumption) and LaTeX (for the
    report appendix table). Columns: overall mean/std, mean split by
    target, median, skewness, kurtosis. Kurtosis in particular flags
    the heavy-tailed features (BILL_AMT, PAY_AMT) that benefit from
    the log / sign-log transforms in preprocessing.
    """
    stats_list = []
...
def plot_repayment_ratio(df: pd.DataFrame, save_dir: str):
    """Figure 13: repayment ratio (PAY_AMT / |BILL_AMT|) analysis.

    (a) September repayment-ratio distribution by default class,
        clipped to ``[0, 2]`` because the tail is extreme (division by
        small bills produces huge ratios).
    (b) Median repayment ratio over six months -- defaulters sit
        below 0.2 consistently, non-defaulters above 1.0 (paid-in-full).
    """
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    ...
    fig.suptitle(
        "Repayment Ratio Analysis - Defaulters Consistently Underpay",
        fontsize=12,
        fontweight="bold",
        y=1.02,
    )
...
def run_eda(
    ...
    stats_dir: Optional[str] = "results/analysis",
    *,
    mode: str = "auto",
    allow_fallback: bool = True,
):
    """End-to-end EDA: load, clean, render all twelve figures + summary table.

    Parameters
    ----------
    data_path : str or None
        Optional override for the raw CSV path; ``None`` defers to
        :func:`src.data.preprocessing.load_raw_data` (which itself
        resolves through :mod:`src.data.sources`).
    save_dir : str
        Figure-output directory. Created if missing. Figures land as
        ``fig01_class_distribution.png`` etc.
    stats_dir : str or None
        Separate directory for the tabular summary artefacts
        (``summary_statistics.csv`` + ``.tex``). These are *results*
        rather than figures, so defaulting them to ``results/analysis``
        keeps the new repo structure tidy. Pass ``None`` to fall back
        to ``save_dir`` (legacy behaviour used by some older notebook
        runs).
    mode, allow_fallback : str, bool
        Forwarded to :mod:`src.data.sources` -- controls cache behaviour
        and fallback to the bundled copy if the live UCI fetch fails.

    Returns
    -------
    (df, stats_df) : (DataFrame, DataFrame)
        The cleaned frame and the summary-stats frame, handy for
        notebook use without re-reading the CSVs.
    """
    os.makedirs(save_dir, exist_ok=True)
    stats_target = stats_dir if stats_dir is not None else save_dir
    os.makedirs(stats_target, exist_ok=True)
    set_publication_style()

    from ..data.preprocessing import clean_categoricals, load_raw_data, normalise_schema
    ...
    stats_df = generate_summary_statistics(df, stats_target)

    print(f"\n{'='*60}")
    print(f"EDA COMPLETE -- 12 figures in {save_dir}/; summary stats in {stats_target}/")
    print(f"{'='*60}")
