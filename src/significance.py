...
def mcnemar_test(
    ...
    * ``c`` = rows where A is wrong and B is correct.
    ...
    if n_discordant == 0:
        return TestResult(
            test="mcnemar", model_a=model_a, model_b=model_b,
            metric="accuracy", statistic=0.0, p_value=1.0, effect=0.0,
            n=len(y), extra={"b": b, "c": c, "method": "no_discordant"},
        )
    ...
    return TestResult(
        test="mcnemar", model_a=model_a, model_b=model_b,
        metric="accuracy", statistic=float(stat), p_value=p_val,
        effect=float(effect), n=len(y),
    ...
    *,
    ...
    if var_diff <= 0:
        ...
        return TestResult(
            ...
            ci_low=effect, ci_high=effect, n=len(y),
        )
    ...
    return TestResult(
        test="delong", model_a=model_a, model_b=model_b,
        metric="auc_roc", statistic=float(z), p_value=p_val,
        effect=effect, ci_low=float(effect - ci_half),
        ci_high=float(effect + ci_half), n=len(y),
        extra={"auc_a": float(auc_a), "auc_b": float(auc_b),
               "var_diff": var_diff},
    )
...
def paired_bootstrap(
    ...
    *,
    ...
    p_val = min(p_val, 1.0)
    return TestResult(
        test="paired_bootstrap", model_a=model_a, model_b=model_b,
        metric=metric_name, statistic=observed, p_value=p_val, effect=observed,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--output-dir", type=Path, default=Path("results/significance"))
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for delta in (0.005, 0.01, 0.02):
        ...
        power_rows.append({
            "auc_gap": delta, "n_required_80pct_power": n_req,
            "prevalence": prevalence, "current_n": len(loaded[0]["y_true"]),
        })
    power_df = pd.DataFrame(power_rows)
    power_df.to_csv(args.output_dir / "power_analysis.csv", index=False)

    plot_pvalue_heatmap(df, args.figures_dir / "significance_pvalue_heatmap.png",
                        test="delong")
    ...
    cols = ["test", "model_a", "model_b", "metric", "statistic",
            "p_value", "effect", "ci_low", "ci_high", "q_value"]
    present = [c for c in cols if c in df.columns]
    print(df.loc[mask, present].to_string(index=False,
