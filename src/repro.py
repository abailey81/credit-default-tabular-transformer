"""Repro checker / CI gate. Regenerates deterministic artefacts into a scratch
dir, diffs vs committed, then checks split hashes + python/torch pins. Exits
non-zero on any failure. Taxonomy: docs/REPRODUCIBILITY.md."""
...
def _compare_dataframes(
    ...
    for col in a.columns:
        ...
        try:
            ...
            # non-numeric → string compare (handles "—", "0.7797 ± 0.0023")
...
def check_transformer_run_files(repo: Path) -> Check:
    """every seed_* dir must carry the files downstream modules consume."""
...
def check_evaluate_regenerates(repo: Path, scratch: Path) -> Check:
    """regen comparison_table.csv, diff vs committed."""
...
def check_rf_predictions_regenerate(repo: Path, scratch: Path) -> Check:
    """regen RF test_predictions.npz, diff bitwise vs committed."""
...
def check_split_hashes_match(repo: Path) -> Check:
    """SHA-256 on split files vs SPLIT_HASHES.md. miss = pipeline drift,
    downstream metrics unsafe."""
    import re
...
def check_python_pins(repo: Path) -> Check:
    """pyproject must pin python + torch."""
...
def check_git_clean(repo: Path) -> Check:
    """working-tree status. informational only, never fails."""
