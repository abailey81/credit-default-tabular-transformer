...
Phase 14A §16.5.3 commits to shipping SHA-256 hashes of every committed
pre-processed data split so markers / peer reviewers can verify they
obtained **the same bytes** as us before comparing metrics.

Hashes were computed on the `feature/phase-11-12-14` branch and are
regenerated by `src/repro.py` on every run (the `split_hashes_match`
check fails if any file drifts).
...
```bash
python src/repro.py
# Look for the [PASS] split_hashes_match line.
```
...
A hash mismatch means the split was regenerated under different
`data_preprocessing.py` behaviour, a different raw-data fetch, or the
file has been modified locally. In all three cases the published
metrics are not directly comparable; regenerate from
